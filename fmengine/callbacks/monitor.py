...
def speed_monitor(elapsed_time, current_step, current_loss, configs, engine):
    if current_step % configs['trainer']['log_steps'] == 0:
        ...
        logger_rank0.info(f"{configs['deepspeed']['train_batch_size'] * configs['trainer']['max_seq_len']/elapsed_time:.2f} tokens/s")

def wandb_monitor(elapsed_time, current_step, current_loss, configs, engine):
    tps = configs['deepspeed']['train_batch_size'] * configs['trainer']['max_seq_len']/elapsed_time
