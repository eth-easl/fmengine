...
def _fwd_preprocess_cumsum_gk(
    ...
    tl.store(
        GK_last_exp_ptr, tl.exp(cumsum).to(GK_last_exp_ptr.dtype.element_ty), mask=mask
    )
...
def _bwd_preprocess_cumsum_gk(
    ...
    gk_last = tl.load(
        GK_cumsum_ptr + (CHUNK_SIZE - 1) * D_MODEL_K, mask=mask, other=0
    ).to(tl.float32)
    ...
    for idx in range(CHUNK_SIZE - 1, -1, -1):
        ...
        grad_k = tl.exp(gk_last - gk_cs) * tl.load(
            DK_reduce_ptr, mask=mask, other=0
        ).to(tl.float32)
