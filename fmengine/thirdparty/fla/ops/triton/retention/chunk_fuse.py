...
def fused_chunk_retention_fwd_kernel(
    ...
    d_b, d_o, d_h = (
        tl.math.exp2(BT * b_b),
        tl.math.exp2(o_i * b_b),
        tl.math.exp2((BT - o_i) * b_b),
    )
...
def fused_chunk_retention_bwd_kernel(
    ...
    for i in range(0, tl.cdiv(T, BT)):
        ...
        # [BT, DK]
        b_dq = tl.dot(b_dd, b_h.to(b_k.dtype), allow_tf32=False) + tl.dot(
            b_ds, b_k, allow_tf32=False
        )
        # [DV, DK]
        b_h = d_b * b_h + tl.dot(
    ...
    for i in range(1, tl.cdiv(T, BT) + 1):
        ...
        # [BT, DK]
        b_dk = (
            tl.dot(b_v, tl.trans(b_dh).to(b_v.dtype), allow_tf32=False) * d_k[:, None]
        )
