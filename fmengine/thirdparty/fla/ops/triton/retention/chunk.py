...
def chunk_retention_fwd_kernel_h(
    ...
    p_h = tl.make_block_ptr(
        h + i_bh * s_hh, (TDK, DV), (s_ht, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0)
    )
...
def chunk_retention_fwd_kernel_o(
    ...
    # [BT, BD]
    b_o = tl.dot((b_q * d_i[:, None]).to(b_q.dtype), b_h, allow_tf32=False) + tl.dot(
        b_s.to(b_q.dtype), b_v, allow_tf32=False
    )
...
def chunk_retention_bwd_kernel_dqkv(
    ...
    # [BT, BD]
    b_dq = tl.dot(
        (b_do * (d_q * scale)[:, None]).to(b_k.dtype), b_h, allow_tf32=False
    ...
    b_ds = tl.trans(b_ds)
    # [BT, BD]
    b_dk = tl.dot(b_v, tl.trans(b_dh), allow_tf32=False) * d_k[:, None] + tl.dot(
        b_ds, tl.trans(b_q), allow_tf32=False
    )
