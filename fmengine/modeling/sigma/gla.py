# credits: https://github.com/berlino/gated_linear_attention/blob/main/gla.py
...
from fmengine.modeling.sigma.intra_chunk.fn import intra_chunk_onc
from fmengine.modeling.sigma.inter_chunk.fn import inter_chunk_onc
...
class GatedLinearAttention(nn.Module):
    def __init__(self, d_model, n_head, use_gk=True, use_gv=False):
        ...
        if use_gk and use_gv:
            ...
            self.v_gate = nn.Sequential(
                ...
                nn.Linear(self.embed_dim // 8, self.embed_dim * self.factor),
            )
        ...
        # ~D^2
            ...
            self.k_gate = nn.Sequential(
                nn.Linear(self.embed_dim, 16, bias=False),
        ...
        # D^2
            ...
            self.k_proj = nn.Linear(self.embed_dim, self.embed_dim // 4, bias=False)
            self.v_gate = nn.Sequential(
                nn.Linear(self.embed_dim, self.embed_dim // 4, bias=False),
                nn.Linear(self.embed_dim // 4, self.embed_dim * self.factor),
            )
    ...
    def gated_linear_attention(
        ...
        q = rearrange(
            q, "b (n c) (h d) -> b h n c d", h=num_head, c=chunk_size
        ).contiguous()
        k = rearrange(
            k, "b (n c) (h d) -> b h n c d", h=num_head, c=chunk_size
        ).contiguous()
        v = rearrange(
            v, "b (n c) (h d) -> b h n c d", h=num_head, c=chunk_size
        ).contiguous()

        if gk is not None:
            gk = rearrange(
                gk, "b (n c) (h d) -> b h n c d", h=num_head, c=chunk_size
            ).contiguous()
