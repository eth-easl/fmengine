...
from fmengine.constants import CHECKPOINT_VERSION
...
from fmengine.parallel.parameters import NanotronParameter, ShardedInfo, SlicesPair
from fmengine.serialize.metadata import CheckpointMetadata, TensorMetadata, load_meta
from fmengine.serialize.utils import (
    ObjectType,
...
def save_weights(
    ...
    for name, param_or_buffer in tqdm(
        ...
        if isinstance(param, NanotronParameter):
            ...
            try:
                save_file(
                    tensors={"data": param_or_buffer}, filename=path, metadata=metadata
                )
