...
def all_gather_batches(
    in_tensor: torch.Tensor, in_split: Sequence[int], group: dist.ProcessGroup
) -> torch.Tensor:
    # All gather along first dimension, allow un-equal splits
    out_tensor = torch.empty(
        (sum(in_split),) + in_tensor.shape[1:],
...
class TopPSampler(Sampler):
    ...
    def __call__(self, sharded_logits: torch.Tensor) -> torch.Tensor:
        ...
        in_split = tuple(
            (
                min_shard_batch_size + 1
                if rank < nb_shard_containing_extra_one
                else min_shard_batch_size
            )
        ...
        local_sharded_logits_out = list(
            torch.split(sharded_logits_out, out_split, dim=0)
        )

        dist.all_to_all(
            local_sharded_logits_out, local_sharded_logits_in, group=self.pg
        )

        logits = torch.cat(
            local_sharded_logits_out, dim=-1
        )  # [sharded_batch_size, vocab_size]

        probs = torch.softmax(
            logits.to(dtype=torch.float) / self.temperature, dim=-1
        ...
        cumulative_probs = torch.cumsum(
            sorted_probs, dim=-1, dtype=torch.float
        )  # [batch_size, vocab_size]
        ...
        gathered_new_decoder_input_ids = all_gather_batches(
            sampled_indices, in_split, group=self.pg
        )
...
class GreedySampler(Sampler):
    ...
    def __call__(self, sharded_logits: torch.Tensor) -> torch.Tensor:
        ...
        max_values, max_indices = sharded_logits.max(
            dim=-1, keepdim=True
        )  # [batch_size, 1]
        ...
        sharded_max_values = torch.cat(
            local_max_values_out, dim=-1
        )  # [sharded_batch_size, num_shards]
        sharded_max_indices = torch.cat(
            local_max_indices_out, dim=-1
        )  # [sharded_batch_size, num_shards]

        # Find global max logit across all shards
        # Note that max is deterministic, and always takes the first one.
        # [sharded_batch_size, 1]
        _global_max_values, global_max_indices = sharded_max_values.max(
            dim=-1, keepdim=True
        )
        ...
        # All gather the new decoder input ids along batch dimension
        gathered_new_decoder_input_ids = all_gather_batches(
            sharded_selected_tokens, in_split, group=self.pg
        )
...
class TopKSampler(Sampler):
    ...
    def __call__(self, sharded_logits: torch.Tensor) -> torch.Tensor:
        ...
        local_top_k_values, local_top_k_indices = torch.topk(
            sharded_logits, self.k, dim=-1
        )

        # Add offset to the indices
        local_top_k_indices = local_top_k_indices + (
            dist.get_rank(self.pg) * vocab_per_shard
        )
        ...
        local_top_k_values_out = list(
            torch.split(top_k_values_out_mat, out_split, dim=0)
        )
        local_top_k_indices_out = list(
            torch.split(top_k_indices_out_mat, out_split, dim=0)
        )
        ...
        # Concat assumes that the primary dimension is the same across all shards
        sharded_local_top_k_values = torch.cat(
            local_top_k_values_out, dim=-1
        ...
        probs = torch.softmax(
            sharded_top_k_values.to(dtype=torch.float) / self.temperature, dim=-1
        )
        ...
        new_decoder_input_ids = sharded_top_k_indices.gather(
            -1, sampled_indices
        )  # [sharded_batch_size]

        # All gather the new decoder input ids along batch dimension
        gathered_new_decoder_input_ids = all_gather_batches(
            new_decoder_input_ids, in_split, group=self.pg
        )
...
class BasicSampler(Sampler):
    ...
    def __call__(self, sharded_logits: torch.Tensor) -> torch.Tensor:
        ...
        probs = torch.softmax(
            logits.to(dtype=torch.float), dim=-1
        )  # [batch_size, vocab_size]
